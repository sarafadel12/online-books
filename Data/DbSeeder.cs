...
public class DbSeeder
{
    public static async Task SeedDefaultData(IServiceProvider service)
    {
        try
        {
            ...
            var secondAdmin = new IdentityUser
            {
                UserName = "secondadmin@gmail.com",
                Email = "secondadmin@gmail.com",
                EmailConfirmed = true
            };

            var secondAdminExists = await userMgr.FindByEmailAsync(secondAdmin.Email);
            if (secondAdminExists is null)
            {
                await userMgr.CreateAsync(secondAdmin, "Admin@123");
                await userMgr.AddToRoleAsync(secondAdmin, Roles.Admin.ToString());
            }


            var userExists = await userMgr.FindByEmailAsync(normalUser.Email);
            if (userExists is null)
            {
                await userMgr.CreateAsync(normalUser, "User@123");
                await userMgr.AddToRoleAsync(normalUser, Roles.User.ToString());
            }
